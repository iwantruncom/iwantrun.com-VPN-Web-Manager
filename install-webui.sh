...
download_project(){
  ...
  SRC_DIR="$(find "$TMP_DIR" -mindepth 1 -maxdepth 1 -type d | while read -r d; do [[ -d "$d/app" && -f "$d/requirements.txt" ]] && echo "$d" && break; done)"
  [[ -z "$SRC_DIR" ]] && die "没有找到解压后的项目目录，请确认仓库根目录包含 app/ 和 requirements.txt。"
